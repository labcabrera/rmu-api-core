...
const percentTable: { min: number; max: number; result: AbsoluteManeuverResult }[] = [
  ...
  },
  {
    min: 176,
    max: Infinity,
    result: { result: 'absolute_success', message: 'You succeed in the best way possible, and impress everyone who sees you.' },
  },
];

export class AbsoluteManeuverService {
  execute(roll: number): AbsoluteManeuverResult {
