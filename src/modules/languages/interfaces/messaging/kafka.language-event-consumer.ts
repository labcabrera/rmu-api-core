...
import { UpdateLanguageRealmCommand } from '../../application/cqrs/commands/update-language-realm.command';

@Controller()
export class KafkaLanguageEventConsumer {
  private readonly logger = new Logger(KafkaLanguageEventConsumer.name);
  ...
  async handleRealmUpdated(@Payload() event: DomainEvent<RealmProps>, @Ctx() context: KafkaContext) {
    ...
    const command = new UpdateLanguageRealmCommand(realm.id, realm.name, realm.owner, realm.accessType);
